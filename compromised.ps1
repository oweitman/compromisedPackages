...
<#
  ...
  Behaviour change: pauses before exit by default (so output remains visible).
  Use -AutoExit in CI to not pause.
...
if ($compromised.Keys.Count -eq 0) { Write-Host "Nothing to check (empty list)."; exit 0 }
...
if ($hitMap.Count -eq 0) {
  ...
  Write-Host ""; Write-Host ("Total: {0} match(es) in {1} file(s)." -f $totalHits, $lockfiles.Count)
