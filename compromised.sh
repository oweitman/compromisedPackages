#!/usr/bin/env bash
# Sicheres NPM Lockfile-Prüfskript auf kompromittierte Pakete
set -o pipefail
...
# Fortschrittsanzeige (10%-Schritte, kein Scrollen im TTY)
...
# Gesamtanzahl Checks (Lockfiles × Anzahl Versionen)
total_versions=0
...
# SED-Escape-Helfer (escapt Regex-Sonderzeichen)
escape_re() { sed 's/[.[\*^$()+?{}|]/\\&/g'; }
...
for file in "${lockfiles[@]}"; do
  for pkg in "${!compromised[@]}"; do
    ...
    for ver in "${vers[@]}"; do
      ...
      while (( percent >= next_threshold )); do
        progress_print "$next_threshold"
        (( next_threshold += 1 ))
        (( next_threshold > 100 )) && break
      done

      pkg_re=$(printf '%s' "$pkg" | escape_re)
      ver_re=$(printf '%s' "$ver" | escape_re)

      # Variante 1: JSON-Eintrag  "name": "…ver…"
      ...
      # Variante 2: yarn.lock-Zeile  name@ver
